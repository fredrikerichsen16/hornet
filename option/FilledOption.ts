...
export class FilledOption extends Option {
     ...
     */
    static decompose(input : string) : [string, FilledOption[]]
    {
        /**
         * Extract command
         */
        ...
        if(flagsRegex && flagsRegex.length > 0) { // if any flags were found
            for(let flag of flagsRegex) { // iterate over flags
                ...
                if(result) {
                    ...
                    let args : any[] = [null, null, null, null];
        ...
        return [<string> command, <FilledOption[]> flags];
    }
     ...
     * @return   [description]
     */
    static returnCorrectDataType(s : string) : [types.primitive, string]
    {
