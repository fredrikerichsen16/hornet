...
export class Option {
     ...
     * }
     */
    static decompose(input : string) : OptionObject
    {
        input = input.trim();
        let returnObj : OptionObject = {
            command: '',
            arguments: [],
            flags: {}
        };
     ...
     */
    static convertValueToType(datatype : string, value : string)
    {
        switch(datatype)
        {
            case 'boolean':
                ...
                break;
            case 'number':
                let intVal = parseFloat(<string> value);
                if(!isNaN(intVal)) {
                    return intVal;
                } else {
                    return undefined;
     ...
     * @return                [description]
     */
    static validOptions(activeCmd : Command, options : OptionObject) {
        let validOptions: FreeObjectLiteral = {};
        ...
        for(let arg of commandArgs) {
            ...
            else
            {
                ...
                if(value)
                {
                    let convertedValue : any = Option.convertValueToType(<string> arg.type, <string> value);
        ...
        for(let flagName in passedFlags) {
            ...
            let correctCommandFlag = find(commandFlags, (flag : any) => {
